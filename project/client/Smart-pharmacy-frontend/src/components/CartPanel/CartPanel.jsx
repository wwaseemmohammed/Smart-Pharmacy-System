  ...
  return (
    <aside className="w-72.5 min-w-72.5 bg-bg-primary border-l border-border-light overflow-hidden flex flex-col flex-shrink-0" style={{ height: 'calc(100vh - 56px)', position: 'sticky', top: '56px' }}>
      ...
      <div className="p-4 px-5 border-t border-border-light">
        {!isEmpty && (
          <button
            className="w-full py-2 bg-coral-50 text-coral-600 border border-coral-100 rounded-md text-xs font-semibold transition-colors mb-3 hover:bg-coral-100"
            onClick={() => {
              Object.keys(cart).forEach(id => onRemove(Number(id)));
            }}
          >
            Clear Cart
          </button>
