...
export default function Topbar({ view, setView, cartCount, onCartToggle }) {
  return (
    <header className="sticky top-0 z-100 h-14 bg-bg-primary border-b border-border-light flex items-center justify-between px-6 gap-4">
      <div className="flex items-center gap-2 text-sm font-bold text-teal-600 tracking-tight">
        <div className="w-7 h-7 bg-teal-400 rounded-md flex items-center justify-center flex-shrink-0"><Pill size={16} color="white" /></div>
      ...
      <button className="relative flex items-center gap-1.5 px-3.5 py-1.5 border border-border-medium rounded-md bg-bg-primary text-xs font-medium text-text-primary transition-all hover:bg-bg-secondary hover:border-teal-400 hover:text-teal-600" onClick={onCartToggle}>
        <ShoppingCart size={15} />
        <span>Cart</span>
        {cartCount > 0 && <span className="absolute -top-1.25 -right-1.25 bg-teal-400 text-white w-4.25 h-4.25 rounded-full text-[10px] font-bold flex items-center justify-center border-2 border-bg-primary">{cartCount}</span>}
      </button>
    </header>
  );
}
